...
// Codex 代理配置
const CODEX_UPSTREAM_URL =
  config.CODEX_UPSTREAM_URL ||
  process.env.CODEX_UPSTREAM_URL ||
  "https://yunyi.cfd/codex";
const CODEX_API_KEY =
  config.YUNYI_API_KEY ||
  process.env.YUNYI_API_KEY ||
  config.OPENAI_API_KEY ||
  process.env.OPENAI_API_KEY;
const CODEX_MODEL = (() => {
  const raw = String(
    config.CODEX_MODEL || process.env.CODEX_MODEL || "gpt-5.2-codex",
  ).trim();
  return ALLOWED_CODEX_MODELS.has(raw) ? raw : "gpt-5.2-codex";
})();
const CODEX_REASONING = (() => {
  const raw = String(
    config.CODEX_REASONING || process.env.CODEX_REASONING || "medium",
  ).toLowerCase();
  return ALLOWED_REASONING.has(raw) ? raw : "medium";
})();
...
const CLAUDE_PROXY_MODE = (
  config.CLAUDE_PROXY_MODE ||
  process.env.CLAUDE_PROXY_MODE ||
  "codex"
).toLowerCase();
...
const PORT = parseInt(
  config.CLAUDE_PROXY_PORT || process.env.CLAUDE_PROXY_PORT || "3457",
  10,
);
const HOST =
  config.CLAUDE_PROXY_HOST || process.env.CLAUDE_PROXY_HOST || "127.0.0.1";
const UPSTREAM_TIMEOUT_MS = parseInt(
  config.UPSTREAM_TIMEOUT_MS || process.env.UPSTREAM_TIMEOUT_MS || "120000",
  10,
);
...
const CLAUDE_CODE_COMPAT = !["0", "false", "off"].includes(
  String(
    config.CLAUDE_CODE_COMPAT || process.env.CLAUDE_CODE_COMPAT || "1",
  ).toLowerCase(),
);
...
const CLAUDE_PROXY_MAX_CONCURRENCY = (() => {
  const raw =
    config.CLAUDE_PROXY_MAX_CONCURRENCY ||
    process.env.CLAUDE_PROXY_MAX_CONCURRENCY ||
    "0";
...
const CLAUDE_PROXY_RETRY_429 = !["0", "false", "off"].includes(
  String(
    config.CLAUDE_PROXY_RETRY_429 || process.env.CLAUDE_PROXY_RETRY_429 || "1",
  ).toLowerCase(),
);
const CLAUDE_PROXY_MAX_RETRIES = Math.max(
  0,
  parseInt(
    config.CLAUDE_PROXY_MAX_RETRIES ||
      process.env.CLAUDE_PROXY_MAX_RETRIES ||
      "3",
    10,
  ) || 3,
);
const CLAUDE_PROXY_TOOL_PAYLOAD = String(
  config.CLAUDE_PROXY_TOOL_PAYLOAD ||
    process.env.CLAUDE_PROXY_TOOL_PAYLOAD ||
    "truncate",
).toLowerCase(); // full | truncate | none
const CLAUDE_PROXY_TOOL_PAYLOAD_MAX_CHARS = Math.max(
  0,
  parseInt(
    config.CLAUDE_PROXY_TOOL_PAYLOAD_MAX_CHARS ||
      process.env.CLAUDE_PROXY_TOOL_PAYLOAD_MAX_CHARS ||
      "800",
    10,
  ) || 800,
);
...
function pickUpstreamModel(anthropicModel) {
  if (
    typeof anthropicModel === "string" &&
    ALLOWED_CODEX_MODELS.has(anthropicModel)
  )
...
function pickReasoningEffort(request) {
  ...
  if (typeof raw === "string") {
    ...
    if (ALLOWED_REASONING.has(normalized)) return normalized;
  }
...
function getClaudeAuthToken() {
  if (typeof CLAUDE_AUTH_TOKEN === "string" && CLAUDE_AUTH_TOKEN.trim())
    return CLAUDE_AUTH_TOKEN.trim();
...
function stripHookNoise(text) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("{") && trimmed.endsWith("}")) {
      if (
        trimmed.includes('"suppressOutput"') ||
        trimmed.includes('"hookSpecificOutput"')
      )
        continue;
      if (
  ...
  for (const part of parts) {
    ...
    if (part.type === "text") {
      if (typeof part.text === "string" && part.text)
        chunks.push(stripHookNoise(part.text));
      continue;
    }
    ...
    if (part.type === "tool_use") {
      ...
      if (CLAUDE_PROXY_TOOL_PAYLOAD !== "none") {
        ...
        if (
          ...
          input.length > CLAUDE_PROXY_TOOL_PAYLOAD_MAX_CHARS
        ) {
    ...
    if (part.type === "tool_result") {
      ...
      if (
        CLAUDE_PROXY_TOOL_PAYLOAD === "truncate" &&
        resultText.length > CLAUDE_PROXY_TOOL_PAYLOAD_MAX_CHARS
      ) {
        resultText =
          resultText.slice(0, CLAUDE_PROXY_TOOL_PAYLOAD_MAX_CHARS) + "…";
      }
      chunks.push(
      ...
      const bytes =
        typeof source.data === "string"
          ...
          : typeof source.bytes === "string"
            ...
            : 0;
      chunks.push(
        `[image${mediaType ? ` media_type=${mediaType}` : ""}${kind ? ` source=${kind}` : ""}${bytes ? ` bytes~=${bytes}` : ""}]`,
      );
      continue;
    }
    ...
    try {
      ...
      chunks.push(
        `[${part.type || "unknown_part"} omitted] ${preview.slice(0, 500)}`,
      );
    } catch (e) {
...
function rewriteClaudeCodeSystemForCodex(
  systemText,
...
function describeClaudeMessageContent(content) {
  if (typeof content === "string")
    return { types: ["string"], approxChars: content.length };
  ...
  for (const part of content) {
    ...
    if (part.type === "tool_result" && typeof part.content === "string")
      approxChars += part.content.length;
  }
        ...
        try {
          ...
          if (
            (chunk.type === "response.function_call_arguments.delta" ||
              chunk.type === "response.output_item.delta") &&
            chunk.delta
          ) {
            ...
            const deltaEvent = {
              ...
              delta: {
                ...
                partial_json: String(chunk.delta),
              },
            };
            res.write(
              `event: content_block_delta\ndata: ${JSON.stringify(deltaEvent)}\n\n`,
          ...
          if (
            ...
            const name = chunk.item.name
              ? String(chunk.item.name)
              : fnCalls.get(id)?.name || "";
            const args =
              typeof chunk.item.arguments === "string"
                ? chunk.item.arguments
                : chunk.item.arguments &&
                    typeof chunk.item.arguments === "object"
                  ? JSON.stringify(chunk.item.arguments)
                  : "";
            if (id) {
...
async function handleMessages(req, res) {
  ...
  try {
    ...
    if (CLAUDE_PROXY_DEBUG) {
      ...
      console.log(
        `[Debug] req=${requestId} mode=${CLAUDE_PROXY_MODE} compat=${CLAUDE_CODE_COMPAT} messages=${msgs.length} tail=${JSON.stringify(types)}`,
      );
    }
    ...
    if (CLAUDE_PROXY_MODE === "claude") {
      ...
      if (!token) {
        return jsonResponse(res, 500, {
          ...
          error: {
            type: "api_error",
            message: "CLAUDE_AUTH_TOKEN not configured",
          },
        });
      }
      ...
      console.log(
        `[Claude] req=${requestId} upstream=${CLAUDE_UPSTREAM_URL} stream=${isStream}`,
      );
      ...
      try {
        response = await fetch(
          `${CLAUDE_UPSTREAM_URL.replace(/\/$/, "")}/v1/messages`,
          {
            method: "POST",
            headers: {
      ...
      if (!response.ok) {
        ...
        console.error(
          `[Claude] req=${requestId} upstream error ${response.status}: ${errorText.slice(0, 200)}`,
        );
        return jsonResponse(res, response.status, {
          type: "error",
          error: { type: "api_error", message: errorText.slice(0, 500) },
        });
      }
    ...
    if (!CODEX_API_KEY) {
      return jsonResponse(res, 500, {
        type: "error",
        error: { type: "api_error", message: "YUNYI_API_KEY not configured" },
      });
    }
    ...
    console.log(
      `[Codex] req=${requestId} requested_model=${requestedModel} upstream_model=${upstreamModel} reasoning=${reasoningEffort} stream=${isStream}`,
    );

    const responsesTools = mapAnthropicToolsToResponsesTools(
      request.tools || request?.tool_schema || request?.toolSchema || [],
    );
    const toolNames = new Set(
      responsesTools.map((t) => t?.name).filter(Boolean),
    );
    const toolChoice = pickToolChoice(request);

    // 转换请求格式 (Anthropic Messages → Responses API)
    const openaiMessages = convertMessages(request.messages || []);
    const filteredInput = openaiMessages.map((m) => ({
      type: "message",
      role: m.role,
      content: m.content || "",
    }));
    ...
    const systemText = rewriteClaudeCodeSystemForCodex(
      normalizeSystem(request.system),
      toolNames,
    );
    if (systemText) {
      const prefix = `SYSTEM:\\n${systemText}\\n\\n`;
    ...
    async function fetchCodexOnce(payload) {
      ...
      try {
        return await fetch(
          `${CODEX_UPSTREAM_URL.replace(/\/$/, "")}/v1/responses`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    async function fetchCodex(payload) {
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        const shouldRetry =
          isRetryable && canRetry && (CLAUDE_PROXY_RETRY_429 || !is429);
        ...
        console.warn(
          `[Codex] req=${requestId} retrying status=${status} attempt=${attempt}/${maxAttempts} wait=${delay}ms`,
        );
        await sleep(delay);
      }
      ...
      return {
        response: new Response(lastErrText || "Upstream error", {
          status: 502,
        }),
        errorText: lastErrText || "Upstream error",
      };
    }
    ...
    try {
      ...
      if (!response.ok) {
        console.error(
          `[Codex] req=${requestId} upstream error ${response.status}: ${errorText.slice(0, 200)}`,
        );
        ...
        const mayBeToolsUnsupported =
          ...
          (responsesRequest.tool_choice &&
            (errorText.includes("tool_choice") ||
              errorText.includes("tool choice")));
        if (
          mayBeToolsUnsupported &&
          (responsesRequest.tools || responsesRequest.tool_choice)
        ) {
          ...
          console.warn(
            `[Codex] req=${requestId} retrying without tools/tool_choice`,
          );
          ...
          if (retry.response.ok) {
            ...
            console.error(
              `[Codex] req=${requestId} retry error ${retry.response.status}: ${retryErr.slice(0, 200)}`,
            );
            // 保持原始错误返回（更贴近用户请求）
          }
        }
      ...
      if (isStream) {
          ...
          for (const data of events) {
            ...
            try {
              ...
              if (chunk.type === "response.completed" && chunk.response) {
                ...
                for (const fc of extractFunctionCallsFromResponse(
                  chunk.response,
                )) {
                  if (!fc?.id) continue;
                  const rec = fnCalls.get(fc.id) || {
                    id: fc.id,
                    name: fc.name || "",
                    args: "",
                  };
        ...
        const toolUses = Array.from(fnCalls.values()).map((r) => ({
          id: r.id,
          name: r.name,
          arguments: r.args,
        }));
        const claudeResponse = convertResponseFromText(
          fullText,
          requestedModel,
          toolUses,
        );
...
const server = createServer(async (req, res) => {
  ...
  if (method === "GET" && pathname === "/ready") {
    ...
    if (CLAUDE_PROXY_MODE === "claude") {
      ...
      if (!token)
        missing.push(
          "CLAUDE_AUTH_TOKEN (or ~/.claude/settings.json env.ANTHROPIC_AUTH_TOKEN)",
        );
    } else {
...
server.listen(PORT, HOST, () => {
  ...
  console.log(
    `  Compat:     ${CLAUDE_CODE_COMPAT ? "on" : "off"} (CLAUDE_CODE_COMPAT)`,
  );
  console.log(
    `  Debug:      ${CLAUDE_PROXY_DEBUG ? "on" : "off"} (CLAUDE_PROXY_DEBUG)`,
  );
  if (CLAUDE_PROXY_MODE === "claude") {
    console.log(`  上游服务:   ${CLAUDE_UPSTREAM_URL}`);
    const masked = CLAUDE_AUTH_TOKEN
      ? `${CLAUDE_AUTH_TOKEN.slice(0, 8)}...`
      : "(未设置)";
    ...
    const masked = CODEX_API_KEY
      ? `${CODEX_API_KEY.slice(0, 8)}...`
      : "(未设置)";
