...
// 加载配置（.env 优先，不污染系统环境变量）
function loadConfig() {
    const config = {};

    // 从 .env / .env.local 文件读取（.env.local 优先，后加载覆盖前者）
    try {
...
const YUNYI_CODEX_URL = config.YUNYI_CODEX_URL || process.env.YUNYI_CODEX_URL || 'https://yunyi.cfd/codex';
const CODEX_REASONING = (config.CODEX_REASONING || process.env.CODEX_REASONING || 'medium').toLowerCase();
...
const PORT = parseInt(config.PROXY_PORT || process.env.PROXY_PORT || '3456', 10);
const PROXY_API_KEY = config.PROXY_API_KEY || process.env.PROXY_API_KEY;
const AUTO_KILL_PORT = (config.AUTO_KILL_PORT || process.env.AUTO_KILL_PORT || 'false').toLowerCase() === 'true';
...
const UPSTREAM_TIMEOUT_MS = parseInt(config.UPSTREAM_TIMEOUT_MS || process.env.UPSTREAM_TIMEOUT_MS || '45000', 10);
const LOG_PREVIEW = (config.LOG_PREVIEW || process.env.LOG_PREVIEW || 'false').toLowerCase() === 'true';
const FORCE_SHUTDOWN_MS = parseInt(config.FORCE_SHUTDOWN_MS || process.env.FORCE_SHUTDOWN_MS || '10000', 10);
...
// Codex 可用模型
const DEFAULT_MODEL = config.YUNYI_MODEL || process.env.YUNYI_MODEL || 'gpt-5.2-codex';
const ALLOWED_CODEX_MODELS = new Set(['gpt-5.2', 'gpt-5.2-codex']);

const mapModel = (model) => {
    if (typeof model === 'string' && ALLOWED_CODEX_MODELS.has(model)) return model;
...
function pickReasoningEffort(chatRequest) {
    ...
    return ['low', 'medium', 'high', 'xhigh'].includes(effort) ? effort : CODEX_REASONING;
}
...
const generateRequestId = () => `req_${typeof randomUUID === 'function' ? randomUUID() : Math.random().toString(36).slice(2, 12)}`;
...
async function handleChatCompletions(req, res) {
    ...
    try {
        ...
        try {
            ...
            return jsonResponse(res, 400, {
                error: { message: 'Invalid JSON in request body', type: 'invalid_request' }
            });
        }
        ...
        if (!chatRequest.messages || !Array.isArray(chatRequest.messages) || chatRequest.messages.length === 0) {
            return jsonResponse(res, 400, {
                error: { message: 'messages field is required and must be a non-empty array', type: 'invalid_request' }
            });
        }
        ...
        if (!YUNYI_API_KEY) {
            return jsonResponse(res, 500, {
                error: { message: 'API key not configured. Please set YUNYI_API_KEY in .env', type: 'configuration_error' }
            });
        }
        ...
        const systemText = systemParts.map(s => String(s || '').trim()).filter(Boolean).join('\n');
        if (systemText) {
        ...
        if (LOG_PREVIEW) {
            ...
            if (Array.isArray(firstContent)) firstContent = firstContent.map(c => c.text || c.content || '').join('');
            else if (typeof firstContent === 'object') firstContent = JSON.stringify(firstContent);
            const firstMsgPreview = String(firstContent || '').slice(0, 50);
            console.log(`[Proxy] req=${requestId} ${chatRequest.model || 'default'} → ${responsesRequest.model} | ${msgCount} msgs | "${firstMsgPreview}..."`);
        } else {
            console.log(`[Proxy] req=${requestId} ${chatRequest.model || 'default'} → ${responsesRequest.model} | ${msgCount} msgs`);
        }
        ...
        let response;
        try {
            response = await fetch(`${YUNYI_CODEX_URL.replace(/\/$/, '')}/v1/responses`, {
                method: 'POST',
                headers: {
        ...
        req.upstreamDurationMs = Number(process.hrtime.bigint() - upstreamStart) / 1e6;
        ...
        if (!response.ok) {
            ...
            return jsonResponse(res, response.status, {
                error: { message: `Upstream error: ${errorText.slice(0, 500)}`, type: 'upstream_error' }
            });
        }
        ...
        console.error(`[Proxy] req=${req.requestId || 'unknown'} Error:`, error.message);
        const status = error.message === 'Request body too large' ? 413
            : error.message === 'Request timeout' ? 408
                : error.message === 'Client aborted' ? 499
                    : error.name === 'AbortError' ? 504
                        : 500;
        if (!res.writableEnded) {
            jsonResponse(res, status, { error: { message: error.message, type: 'internal_error' } });
        }
    }
}
...
const server = createServer(async (req, res) => {
    ...
    const logAccess = () => {
        ...
        const upstreamInfo = req.upstreamStatus
            ...
            : '';
        console.log(`[Proxy] req=${requestId} ${method} ${pathname} ${res.statusCode} ${durationMs.toFixed(1)}ms${upstreamInfo}`);
    };
    ...
    if (method === 'GET' && pathname === '/health') {
        return jsonResponse(res, 200, { status: 'ok', service: 'yunyi-proxy-fast' });
    }
...
server.listen(PORT, HOST, () => {
    ...
    const maskedKey = YUNYI_API_KEY ? `${YUNYI_API_KEY.slice(0, 4)}...${YUNYI_API_KEY.slice(-4)}` : '(未设置)';
    console.log(`  云驿 Key:   ${maskedKey}`);
    console.log(`  运行时:     ${typeof Bun !== 'undefined' ? 'Bun 🚀' : 'Node.js'}`);
    console.log('');
